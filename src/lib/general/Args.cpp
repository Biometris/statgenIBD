...
Option::Option(const std::string& k, const std::string& t, const std::string& d)
	: key(k), descr(d)
{
	state = true;
	if (t == "")
...
void Options::Add(const std::string& opt, const std::string& descr)
{
	istringstream i(opt);
	string key,type;
	i >> key >> type;
...
ostream& mbl::operator<<(std::ostream& outp, const Options& options)
{
...
void Args::init(std::string args, std::string opt_arg, int argc, const char *argv[])
{
	read_args(args);
...
Args::Args(const Options& options, std::string args, std::string opt_arg,
...
Args::Args(std::string format, std::string args,
		   std::string opt_arg, int argc, const char *argv[])
{
...
{
	if (!m_state)
		cout << "Error: " << m_error_string << endl;
	cout << endl << opts << endl << endl;
	return 1;
}
