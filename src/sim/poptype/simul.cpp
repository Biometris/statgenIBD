                                              ...
                                              PopulationType poptype)
{
  ...
  {
    if (R::runif(0, 1) < fraction_missing)
      result[loc] = U;
    else
      result[loc] = g[loc];
  }
...
void ibd::sim(vector<double>& pheno, // phenotypic values
              ...
              const QTLModel& QTLmodel,
              const LinkageMap& QTLmap, // QTL info
              double sigma_env, // env. var
              const vector<double>& weight,	// env. weights
              const LinkageMap& markermap, // marker info
              double fr_miss,
              vector<double> chr_length,
              int nind,
              PopulationType poptype)	// population info
{
  ...
  for (int ind = 0; ind < nind; ind++)  // for each individual
  {
    ...
    pheno_value = geno_value + R::rnorm(0.0, sigma);
