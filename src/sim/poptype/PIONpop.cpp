...
PIONpop::PIONpop(alleletype p1, alleletype p2, int generation)						
     : AA(Genotype(p1,p1)), 
	   AB(Genotype(p1,p2)), 
	   BB(Genotype(p2,p2)),
       pop_base(ObsGeno(Genotype(p1,p1),
					   Genotype(p1,p2),
					   Genotype(p2,p2)), 1,make_PION_name(generation)), gen(generation) {}


double PIONpop::p_trans_QTL(const Genotype& g_par, const Genotype& g_prog) const
{
	if (g_par == AB)
		return (g_prog == AB) ? 0.0 : 0.5;

	return (g_prog == g_par) ? 1.0 : 0.0;
}


double PIONpop::p_trans(const Genotype& g1, const Genotype& g2, double r) const
{
...
vector<double> PIONpop::model(const Genotype& g) const
{
	...
	if (g == AA) 
		...
		throw ibd_error("PIONpop::model : unknown genotype g");
	return X;
}

// dit moet nog aangepast/uitgebreid worden!
Genome PIONpop::make_ind(const vector<double>& chr_length) const
{
...
vector<Genome> PIONpop::make_pop(const vector<double>& chr_length, int nind) const
{
	Genome F1(chr_length,AB); // F1 population
...
void PIONpop::MapQTL(ostream& outp, const ObsGeno& g) const
{
...
ObsGeno PIONpop::MapQTL(istream& inp) const
{
	ObsGeno C(AB,BB); // genotype B*
	ObsGeno D(AA,AB); // genotype A*
	char c;
