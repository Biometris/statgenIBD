...
SimPop sim_population(const PopProp& pop,
                      const map<string,Genome>& genome_par,
...
void sim_SS_NS_pedigree(const vector<string>& fnd_names,
                        string filename,
                        int Ngen,
                        int N)
{
  ...
  for (int grp=0;grp<2;grp++)
  {
    ...
    for (int gen=0;gen<Ngen;gen++)
    {
      ...
      for (int i=0;i<N;i++)
      {
        vector<string> sel_namesI = sel_names;
        int P1pos = rand() % sel_namesI.size();
        string P1 = sel_namesI[P1pos];
        sel_namesI.erase(sel_namesI.begin() + P1pos);
        string P2 = sel_namesI[rand() % sel_namesI.size()];
