...
The calculations of the IBDs are based on Hidden Markov Models (HMM) and inheritance vectors. For details of the theory see @Lander1987 and @Huang2011, supplementary material. 
...
Adding the extra evaluation points can be done on a grid by setting `grid = TRUE`. Doing this defines a grid of evaluation point from the beginning of each chromosome to the end with a distance between the evaluation points of `evaldist`. The original marker positions will be ignored in this case. To use evaluation points on a regular grid is important for haplotype construction and can be efficient for detection of QTLs using a scan along the genome [@VanEeuwijk2009].
...
F4IBD <- calcIBD(poptype = "F4",
                 locfile = system.file("extdata/popF4", "cross.txt",
                                       package = "statgenIBD"),
...
C4S3IBD <- calcIBD(poptype = "C4S3",
                   locfile = system.file("extdata/popC4S3", "cross.txt",
                                         package = "statgenIBD"),
