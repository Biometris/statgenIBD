...
Using the map and genotypic files we can compute the IBD probabilities using `calcIBD`. This function computes, per genotype and marker, the probability that it descended from either parent. If this information was known from the input, the probabilities will simply be 0 for one of the parents and 1 for the other. When a value was missing from the input data, it is imputed by the function. The probabilities are then estimated based on the population type and the values for the surrounding markers.
...
The output of the `calcIBD` function is an object of class `calcIBD`. This object is a `list` that consists of four elements:
...
| multiCross | `logical`    | An indicator showing whether in this `calcIBD` object multiple crosses where combined. This is always `FALSE` for the output of the `calcIBD` function, but will be `TRUE` when multiple crosses are combined as we will see in the examples.          |

The `summary` function can be used to get a short description of the content of a `calcIBD` object.
...
For the combination of marker **ABG053** and genotype **dh146** the genotypic file contained a missing value. Looking at the output we can see that the probability that this marker came from Morex is about 0.48 and the probability that it came from Steptoe is about 0.52.
...
SxM_probs <- getQTLProb(SxMIBD, QTLS = c("plc", "ABG053"))
head(SxM_probs)
```

The output of the `getQTLProb` function is a `data.frame` with a column `geno` containing the genotype and for both markers the probability that it descended from Morex and that it descended from Steptoe.
...
First we have a look at an F4 population, i.e. an F1 followed by 3 generations of selfing. This population was constructed from parents GREEN and RED. As we can see from the table in the populations section, when doing computations for an F4 populations, we can use `poptype = "Fx"` where x is replaced by 4.
...
As the summary shows, the output contains probabilities for three parents, GREEN, RED and GREENRED.

### C4S3

Also included is a C4S3 population, a four-way cross followed by 3 generations of selfing. The parents in the four-way cross where YELLOW, BLUE, GREEN and RED.
...
As the summary shows, the output contains probabilities for eight parents, YELLOW, BLUE, GREEN, RED, YELLOWGREEN, YELLOWRED, BLUEGREEN, BLUERED.
...
Now we want to combine the results from both computations for further analyses. To combine to `calcIBD` objects we can use the `c` function. This is only possible if both objects contain the same type of population. Also the evaluation points for both object have to be indentical. In this example we have two F4DH populations and we used the same map file in the computations so combining them is fine.
...
Looking at the summary, we can see the our combined population now has 180 individuals, 100 from AB and 80 from AC. There are also three parents. For all individuals from population AB the probability for parent C will be 0, as will be the probability for parent B for individuals from population AC.

```{r checkMultiIBD}
## Extract probabilities for markers M1_1 and M3_3.
ABCProbs <- getQTLProb(ABC, QTLS = c("M1_1", "M3_3"))
