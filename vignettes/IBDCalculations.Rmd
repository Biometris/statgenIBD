...
### Write and read tab-delimited *.ibd files

The results of IBD calculations can be written to a tab-delimited *.ibd* file using `writeIBDs`. Details about this file format can be found in the accompanying [IBD file format vignette](IBDFileFormat.html).

```{r SxMwriteIBD, eval=FALSE}
## Write IBDs to tab-delimited .ibd file.
writeIBDs(SxMIBD_Ext, "SxMIBD_Ext.ibd")
```
...
## Write IBDs to tab-delimited .ibd file.
SxMIBD_Ext <- readIBDs("SxMIBD_Ext.ibd")
summary(SxMIBD_Ext)
```
